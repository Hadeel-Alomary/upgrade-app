...
export class ComponentsHelper {
  ...
  public appendNextToLocation<T>(ComponentClass:Type<T>,
    ...
    let childInjector: Injector = injector ? injector : location.parentInjector;
  ...
  public appendNextToRoot<T>(ComponentClass:Type<T>,
    ...
    let componentInjector = Injector.create({
        providers: [{provide: ComponentOptionsClass, useValue: options, deps: []}]
    });

    return this.appendNextToLocation(ComponentClass, location, componentInjector);
  }
}
